...
### Dashboard UI v2 feature flag
Enable the new dashboard shell while keeping the current UI as default:

- Query param: open the app with `?ui=v2` (for example `http://localhost:5173/?ui=v2`).
- Env var: set `VITE_UI_V2=true` before starting/building the frontend.

If both are provided, the query param `ui=v2` enables v2 immediately.
