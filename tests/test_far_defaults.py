...
def test_far_autofill_when_not_explicit(monkeypatch):
    ...
    monkeypatch.setattr(estimates_api.far_rules, "lookup_far", lambda *args, **kwargs: 2.5)
    ...
    payload = {
        ...
        "city": "Riyadh",
        "excel_inputs": {**sample_excel_inputs(), "area_ratio": {}},
    }
    ...
    assert pytest.approx(far_info.get("suggested_far") or 0.0, rel=1e-6) == 1.5
...
def test_far_preserved_when_explicit(monkeypatch):
    ...
    monkeypatch.setattr(estimates_api.far_rules, "lookup_far", lambda *args, **kwargs: 2.0)
    ...
    payload = {
        ...
        "city": "Riyadh",
        "far": 3.3,
        "excel_inputs": {**sample_excel_inputs(), "area_ratio": {"residential": 1.0}},
    }
    resp = client.post("/v1/estimates", json=payload)
    assert resp.status_code == 200
    notes = resp.json()["notes"]
    far_info = notes.get("far_inference") or {}
    assert pytest.approx(far_info.get("far_used") or 0.0, rel=1e-6) == 3.3
    built_area = notes.get("excel_breakdown", {}).get("built_area", {})
