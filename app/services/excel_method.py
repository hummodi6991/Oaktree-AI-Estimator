...
def allocate_mixed_use_area_ratio(
    target_far: float, inputs: Dict[str, Any]
) -> tuple[Dict[str, Any], Dict[str, Any]]:
    """
    Option C — Allocate mixed-use program using rent/unit_cost/efficiency.

    Returns a copy of `inputs` with residential/retail/office `area_ratio` set so that
    the above-ground FAR sums to `target_far`. Basement ratios are preserved.
    """
    ...
    area_ratio = dict(result.get("area_ratio") or {})
    rent_map = result.get("rent_sar_m2_yr") or {}
    unit_cost = result.get("unit_cost") or {}
    efficiency = result.get("efficiency") or {}

    scores: dict[str, float] = {}
    weights: dict[str, float] = {}
    components = ("residential", "retail", "office")
    for comp in components:
        rent = _as_float(rent_map.get(comp))
        eff = _as_float(efficiency.get(comp))
        cost = _as_float(unit_cost.get(comp))
        denom = cost if cost > 0 else 1.0
        scores[comp] = (rent * eff) / max(denom, 1.0)

    total_score = sum(v for v in scores.values() if v > 0)
    for comp in components:
        weights[comp] = (scores[comp] / total_score) if total_score > 0 else 0.0

    retail_far_cap = min(0.8, 0.6 * float(target_far))
    office_far_cap = min(0.6, 0.5 * float(target_far))

    retail_far = max(0.0, min(float(target_far) * weights["retail"], retail_far_cap))
    office_far = max(0.0, min(float(target_far) * weights["office"], office_far_cap))
    residential_far = max(float(target_far) - (retail_far + office_far), 0.0)

    # Adjust for any rounding drift so totals match target_far.
    total_allocated = residential_far + retail_far + office_far
    if abs(total_allocated - float(target_far)) > 1e-9 and float(target_far) > 0:
        residential_far = max(residential_far + (float(target_far) - total_allocated), 0.0)
        total_allocated = residential_far + retail_far + office_far

    new_area_ratio: Dict[str, Any] = {}
    for key, value in area_ratio.items():
        key_norm = str(key).strip().lower()
        if key_norm in {"residential", "retail", "office"}:
            continue
        new_area_ratio[key] = value
    new_area_ratio.update(
        {
            "residential": residential_far,
            "retail": retail_far,
            "office": office_far,
        }
    )

    result["area_ratio"] = new_area_ratio
    meta = {
        "target_far": float(target_far),
        "scores": scores,
        "weights": weights,
        "caps": {"retail": retail_far_cap, "office": office_far_cap},
        "allocated_far": {
            "residential": residential_far,
            "retail": retail_far,
            "office": office_far,
